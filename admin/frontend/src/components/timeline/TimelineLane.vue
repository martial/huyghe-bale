...
<template>
  ...
  >
    ...
    <g v-for="point in lane.points" :key="point.id">
      ...
      <!-- Bezier handles -->
      <template v-if="point.bezier_handles && point.curve_type === 'bezier'">
        <g v-if="selectedPointId === point.id">
          <!-- Find previous point for handle rendering -->
          <!-- Handles are shown relative to the segment arriving at this point -->
        </g>
      </template>
    </g>
