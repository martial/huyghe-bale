...
| Alarm lock | `_alarm_isr` + `alarm_locked` flag | **opt-in** via `alarm_polarity` (`active_high` / `active_low` / `disabled`). Default is `disabled` until rig wiring is verified — turning it on with the wrong polarity bricks motion at boot. When enabled and either CL86Y `ALARM` GPIO trips, the firmware: (1) sets `alarm_locked=True`, (2) aborts current motion, (3) disables the driver, (4) refuses every motion command until `/trolley/alarm/reset` is sent. Reset only succeeds while both alarm pins are de-asserted. Status broadcasts `alarm` (live OR) and `alarm_locked` (sticky flag). The admin panel shows a red banner with a Reset button when locked. |
